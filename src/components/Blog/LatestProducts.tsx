          ...
          {products.slice(0, 3).map((product, key) => (
            <div className="flex items-center gap-6" key={key}>
              <div className="flex items-center justify-center rounded-[10px] bg-gray-3 max-w-[90px] w-full h-22.5">
                <Image src={product.imgs?.thumbnails?.[0]} alt="product" width={74} height={74} className="object-contain" />
              </div>
              ...
              <div>
                ...
                <p className="text-custom-sm">Price: Rs. {product.price.toLocaleString("en-PK")}</p>
              </div>
            </div>
          ))}
        </div>
